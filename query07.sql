...
SELECT
    EXTRACT(YEAR FROM start_time) AS trip_year, -- dynamic query to get year from date
    EXTRACT(QUARTER FROM start_time) AS trip_quarter,-- dynamic query to get quarter from date
    COUNT(*) AS num_trips
